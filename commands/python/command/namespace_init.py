...
def parse_args(sub_parser):
    subparser = sub_parser.add_parser('namespace-init',
    ...
    subparser.add_argument('--force', '-f', action='store_true',
                           help=('don\'t check if namespace init projects are already installed '
                                 'before installing'))
...
def namespace_init(namespace, force=False):
    ...
    for project in namespace_init_projects:
        ...
        if not force and helm.release_exists(f'{project_name}-{namespace}'):
            continue
        deploy.deploy(project_name, ref, namespace, dry_run=False,
