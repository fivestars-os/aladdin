...
def parse_args(sub_parser):
    ...
    subparser = sub_parser.add_parser('publish',
                                      help=('Build the docker and helm package and publish to ecr '
                                            'and S3'))
    exclude_steps_group = subparser.add_mutually_exclusive_group()
    exclude_steps_group.add_argument('--build-only', action='store_true',
                                     help=('only builds docker images in your minikube env with '
                                           'hash and timestamp as tags'))
    exclude_steps_group.add_argument('--build-publish-ecr-only', action='store_true',
                                     help='only build and publish docker images to ecr')
    exclude_steps_group.add_argument('--publish-helm-only', action='store_true',
                                     help=('only publish helm chart to s3. WARNING: make sure you '
                                           'build and publish the docker images to ecr before '
                                           'deploying'))
    clean_options_groups = subparser.add_mutually_exclusive_group()
    clean_options_groups.add_argument('--build-local', action='store_true',
                                      help=('do a local docker build rather than pulling cleanly '
                                            'from git'))

    remote_options_group = subparser.add_argument_group('remote options')
    remote_options_group.add_argument('--repo',
                                      help=('which git repo to pull from, which should be used if '
    ...
    remote_options_group.add_argument('--git-ref',
                                      help=('which commit hash or branch or tag to checkout and '
                                            'publish from'))
    remote_options_group.add_argument('--submodule', action='store_true',
                                      help=('recursively update all submodules included in the project'))
...
def publish_args(args):
    if args.build_local:
        ...
        publish_clean(args.build_only, args.build_publish_ecr_only, args.publish_helm_only,
                      args.repo, args.git_ref, args.submodule)
...
def publish_clean(build_only, build_publish_ecr_only, publish_helm_only, repo, git_ref, submodule):
    g = Git()
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        if submodule:
            try:
                g.update_submodules(tmpdirname)
            except subprocess.CalledProcessError:
                logging.warn(f'Could not update submodules. Make sure you use ssh urls in '
