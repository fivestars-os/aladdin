...
def parse_args(sub_parser):
    subparser = sub_parser.add_parser('cluster-init',
                                      help=('Install all projects as defined by cluster_init in '
                                            'your cluster\'s config if not already installed')) 
    subparser.add_argument('--force', '-f', action='store_true',
                           help=('don\'t check if cluster init projects are already installed '
                                 'before installing'))
...
def cluster_init(force=False):
    ...
    for project in cluster_init_projects:
        ...
        if not force and helm.release_exists(f'{project_name}-{project_namespace}'):
            continue
        deploy.deploy(project_name, ref, project_namespace, dry_run=False,
            force=True, repo=repo, set_override_values=[])
