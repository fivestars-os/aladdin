...
def search_certificate_arn(boto_session, dns_name):
    ...
    if len(list_cert) == 1:
        ...
        log.warning(f'Certificate {arn_res} for your namespace is not in issued state yet, so '
...
def new_certificate_arn(boto_session, dns_name):
    ...
    # in issued state and project is redeployed
    log.warning(f'Certificate {arn} for your namespace is not in issued state yet, so '
                 'https will not work.  Please redeploy your project later with the --init flag '
                 'set to enable https.')
    # Return empty string so that load balancer will create, and at least http will work
    return ''
...
def _validate_certificate_with_retry(boto_session, dns_name, arn, max_retries=20):
    ...
    dns_id = get_hostedzone(boto_session, hostedzone_name) or create_hostedzone(boto_session, hostedzone_name)
    ...
    while retry_count < max_retries:
        try:
            cname_info = acm.describe_certificate(CertificateArn=arn)['Certificate']['DomainValidationOptions'][0]['ResourceRecord']
    ...
    if retry_count == max_retries:
        log.warning(f'Unable to validate certificate {arn} via DNS at this time. Please try again '
